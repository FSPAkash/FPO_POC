...
7. `FPO_AUTH_SECRET` is generated by the Blueprint. If you create services manually, set it to a long random value.
8. Leave `OPENAI_AGENT_MODEL` as the default unless you want a different model.
9. Click `Apply`.
10. Wait for the backend build to finish, then the frontend build.
11. Open the frontend `onrender.com` URL and use the app normally.
12. Optional: turn on the GitHub Actions keep-alive by setting the repo variable `RENDER_BACKEND_HEALTH_URL` to your backend health URL, such as `https://your-backend.onrender.com/api/health`.
...
- Environment variables:
  - `FPO_AUTH_SECRET` = long random string used to sign demo login tokens
  - `CORS_ORIGINS` = your frontend public origin, for example `https://fpo-poc-frontend.onrender.com`
  - `OPENAI_API_KEY` = your key
  - `OPENAI_AGENT_MODEL` = `gpt-5.2` (optional)
  - `FPO_DEMO_USERS` = optional JSON object for demo users, for example `{"Akash":"a1234","Naina":"n1234"}`
...
- Do not put `FPO_AUTH_SECRET` or `FPO_DEMO_USERS` on the frontend static site.
- The backend only accepts browser API calls from `CORS_ORIGINS` plus local development origins. If you rename the Render frontend service or add a custom domain, update `CORS_ORIGINS` and redeploy the backend.
